...
;;	(:use org.vi-server.jscfi.jscfi)) 
;;    (def j (connect (get-fake-jscfi-factory) nil "scfi" "test"))
;;    (def t (get-task  j "server-33"))
...
(def ^{:dynamic true} *jscfi-fake* (future (reify JscfiFactory
	...
	(if (and (= address "scfi") (= username "test"))
	 (new FakeJscfi observer [
	    {:name "Qqq" :source-file "qqq.c" :input-file "input.txt" :output-file "output.txt" :id 0.3232 :task-id "server-33" :node-count 4 :status :waiting}
	    {:name "Www" :source-file "www.c" :input-file "input.txt" :output-file "output.txt" :id 0.2342 :task-id "server-34" :node-count 1 :status :running}
	    ])
	 nil)))))
